...
fts(1).t = 'twin'; fts(1).c = 'AreaShape_Area'; fts(1).p = [5,35];
%fts(2).t = 'min'; fts(2).c = 'NumGrans'; fts(2).p = 2;
filterp.fts = fts;
dataloc = JB_DataHandler_V3('extractdata',true,'filterp',filterp);

%% Plot the data
plotme = {'t_granspercell','NumGrans'};%,'GranAreaShape_Area','GranIntegratedGFP' 'GranMeanGFP','t_granspercell','t_count_cells','NumGrans','GranAreaShape_Area','t_granspercell','t_count_cells',
plottype = {'mean'};
